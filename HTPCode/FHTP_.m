...
function [x,S,NormRes,NbIter] = FHTP_(y,A,s,MaxNbIter,mu,NbDesc,t,x0,TolRes,Warnings)
...
if strcmp(Warnings,'On')
    if ( (NormRes>TolRes*normy) || isnan(NormRes) )
        disp(strcat('Warning: FHTP_ did not find the correct solution when using a number of iterations =', num2str(MaxNbIter), '(norm of residual =', num2str(NormRes), ' > TolRes =', num2str(TolRes), ')'));
    end
end
x=diag(d)*x;
