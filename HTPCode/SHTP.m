...
function [X,S,NormRes,NbIter] = SHTP(Y,A,s,MaxNbIter,mu,X0,TolRes,Warnings,Eps)
...
if strcmp(Warnings,'On')
    if sum(S==Snew)<s
        disp(strcat('Warning: SHTP did not converge when using a number of iterations =', num2str(MaxNbIter)));
        disp(strcat('Try using SHTP_, which normalizes the columns of A to have Euclidean norm =1'));
    else 
    if NormRes>TolRes*norm(Y,'fro')
        disp(strcat('Warning: SHTP converged to an incorrect solution (norm of residual =', num2str(NormRes),')'));
        disp(strcat('Try using SHTP_, which normalizes the columns of A to have Euclidean norm =1'));
    end
    end
end
